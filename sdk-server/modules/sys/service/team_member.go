...
func (e *SysMemberService) Query(req dto.SysMemberGetPageReq, list *[]models.SysMember, total *int64) error {
	...
	if req.DeptId > 0 {
		var dept models.SysDept
		if err := SerSysDept.Get(req.DeptId, &dept); err != nil {
			return err
		}
...
func (e *SysMemberService) GetUserTeams(uid int, resp *[]dto.TeamMemberResp) error {
	sql := `Select t.name as team_name, t.owner ,user_id, team_id,nickname, m.name, phone, 
			dept_id, dept_path, post_id, role_id, entry_time, gender,birthday
			From team t,team_member m 
			Where user_id = ? and m.status = 1 and t.status > 0 and m.team_id = t.id 
...
func (e *SysMemberService) Exist(teamId, userId int) (bool, error) {
	cStr, err := e.Cache().Get(ckey.TeamMemberKey(teamId, userId))
	if err == nil && cStr != "" {
		return true, nil
	}
	var cnt int64
...
func (e *SysMemberService) GetMember(teamId, userId int, member *models.SysMember) error {
	...
	e.Cache().Set(ckey.TeamMemberKey(teamId, userId), *member, time.Hour)
	return nil
}
...
func (e *SysMemberService) CountMembers(deptId int) (int64, error) {
	var count int64
	if err := e.DB().Model(&models.SysMember{}).Where("dept_id = ?", deptId).Where("status = ?", 1).Count(&count).Error; err != nil {
		return 0, err
	}
	return count, nil
}

func (e *SysMemberService) Del(id int, teamId int, optionUid int) error {
	var member models.SysMember
	if err := e.Get(id, &member); err != nil {
		return err
	}
	if member.TeamId != teamId {
		return codes.Err403(nil)
	}
	member.UpdateBy = optionUid
	member.Status = enums.STATUS_DEL
	return e.DB().Updates(member).Error
}
