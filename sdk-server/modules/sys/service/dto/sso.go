...
type BaseInfo struct {
	...
	DevId string `json:"devId"` // 设备id 客户端id
	Ver   string `json:"ver"`   // 客户端版本
	Src   string `json:"src"`   // 来源渠道
	TID   string `json:"tid"`   // 跟踪代码
}

type GetOneClickTokenReq struct {
	//Platform    string `json:"platform"`    //应用平台类型 Android|iOS
	//PackageName string `json:"packageName"` //应用包名 ios传BundleId
	AccessCode string `json:"accessCode"` //一键登录访问令牌
	Phone      string `json:"phone"`      //脱敏手机号
	OutId      string `json:"outId"`      //ali平台外部id
}

type GetOneClickTokenResp struct {
	VerifyResult string `json:"verifyResult"`
	VerifyId     string `json:"verifyId"`
}

type GetFusionTokenReq struct {
	Platform    string `json:"platform"`    //应用平台类型 Android|iOS
	PackageName string `json:"packageName"` //应用包名 ios传BundleId
}
...
type OneClickLoginReq struct {
	BaseInfo
	VerifyToken string `json:"verifyToken"` //验证token
	Platform    string `json:"platform"`    //应用平台类型 Android|iOS
	Carrier     string `json:"carrier"`     //腾讯平台
	OutId       string `json:"outId"`       //ali平台 一键登录
}
...
type LoginOK struct {
	...
	RefreshExpire time.Time `json:"refreshExpire"` //刷新token有效期
