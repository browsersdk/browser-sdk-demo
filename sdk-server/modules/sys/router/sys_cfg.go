...
func init() {
	routerCheckPermRole = append(routerCheckPermRole, registerSysCfgRouter)
}

// 默认需登录认证的路由
func registerSysCfgRouter(v1 *gin.RouterGroup) {
	r := v1.Group("cfg") //.Use(middleware.JwtAdminHandler())
	{
		...
		r.POST("upload", apis.ApiUploadLog.Upload)
		//r.POST("tts", apis.ApiSysCfg.TTS)
	}
}
