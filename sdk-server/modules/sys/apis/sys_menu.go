...
func (e *SysMenuApi) Get(c *gin.Context) {
	...
	if _, err := service.SerSysMenu.GetAndApi(req.Id, &data); err != nil {
		e.Error(c, err)
		return
	}
	e.Ok(c, data)
...
// @Success 200 {object} base.Resp{data=models.SysMenu} "{"code": 200, "data": [...]}"
// @Router /api/sys/menu/create [post]
// @Security Bearer
func (e *SysMenuApi) Create(c *gin.Context) {
	...
	currentUserId := utils.GetAdminId(c)
...
// @Product application/json
// @Success 200 {object} base.Resp{data=[]models.SysMenu} "{"code": 200, "data": [...]}"
// @Router /api/sys/menu/grant [post]
// @Security Bearer
...
// @Router /api/sys/user/menus [post]
// @Security Bearer
func (e *SysMenuApi) GetUserMenus(c *gin.Context) {
	adminId := utils.GetAdminId(c)
	var ms []dto.MenuVo
	if err := service.SerSysMenu.GetAdminMenus(adminId, &ms); err != nil {
		e.Error(c, err)
		return
	}
	e.Ok(c, ms)
...
// @Router /api/sys/menu/set_apis [post]
