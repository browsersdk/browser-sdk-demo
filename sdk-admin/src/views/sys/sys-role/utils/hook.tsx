...
export function useSysRole() {
  ...
  function handleDelete(row) {
    delSysRole({ ids: [row.id], teamId: teamId.value }).then(res => {
      if (res.code == 200) {
  ...
  function openDialog(type, row?: SysRoleFormItemProps) {
    addDialog({
      ...
      beforeSure: (done, { options }) => {
        ...
        //const mids = curData.halfMenuIds.concat(curData.menuIds);
        const mids = curData.menuIds;
  ...
  onMounted(() => {
    if (router.currentRoute.value.query.id) {
      teamId.value = parseInt(router.currentRoute.value.query.id as string, 10);
    }
    getMenus();
    onSearch();
  });
