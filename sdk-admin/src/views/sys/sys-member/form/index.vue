...
const { genderOptions, postOptions, teamId, roleOptions } = useMember(tableRef, treeRef);
...
//     page: 1,
//     pageSize: 10000,
//     teamId: props.formInline.teamId
//   });
//   //console.log("getRolePage data", data);
//   roleList.value = data.list || [];
//   //console.log("getRolePage r", roleList.value);
    ...
    getSysMember({ id: newFormInline.value.id }).then(res => {
      if (res.code === 200) {
        ...
        newFormInline.value.postId =
          res.data.postId == 0 ? 6 : res.data.postId;
        newFormInline.value.id = res.data.id;
        newFormInline.value.status = res.data.status;
        newFormInline.value.teamId = res.data.teamId;
        newFormInline.value.roleId =
          res.data.roleId == 0 ? null : res.data.roleId;
        getRolePage();
      }
    });
  }
...
const handleRadioChange = row => {
  userInTeam({
    ...
    if (!res.data.exist) {
      ...
      newFormInline.value.name = row.name || null;
    } else {
      message("当前用户已在团队中", {
        type: "error"
      });
