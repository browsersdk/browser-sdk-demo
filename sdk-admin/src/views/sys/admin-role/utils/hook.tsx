...
import { type AdminRoleFormItemProps } from "@/api/sys/admin-role";
import { type PaginationProps } from "@pureadmin/table";
import { reactive, ref, onMounted, h, toRaw } from "vue";
import { cloneDeep, isAllEmpty } from "@pureadmin/utils";
import { handleTree } from "@/utils/tree";
import { getSysMenuGrant } from "@/api/sys/sys-menu";
import { statusSimpleOptions } from "@/api/sys/common";
...
export function useAdminRole() {
  ...
  const columns: TableColumnList = [
    ...
    },
    {
      ...
      minWidth: 120,
      formatter: ({ status }) => {
        return statusSimpleOptions.find(item => item.value === status)?.label;
      }
    },
    {
      label: "创建者",
  ...
  function handleDelete(row) {
    delAdminRole({ ids: [row.id] }).then(res => {
      if (res.code == 200) {
  ...
  async function onSearch() {
    loading.value = true;
    const { data } = await getAdminRolePage(toRaw(form));
  ...
  function openDialog(type, row?: AdminRoleFormItemProps) {
    addDialog({
      ...
      beforeSure: (done, { options }) => {
        ...
        const curData = options.props.formInline as AdminRoleFormItemProps;
