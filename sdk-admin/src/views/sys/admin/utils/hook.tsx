...
import { getAdminRoleList } from "@/api/sys/admin-role";

import { AdminDeptFormItemProps, getAdminDeptTree } from "@/api/sys/admin-dept";
import { genderOptions, findPathById } from "@/api/sys/common";
...
export function useMember(tableRef: Ref, treeRef: Ref) {
  ...
  const deptData = ref(Array<AdminDeptFormItemProps>);
  const treeData = ref([]);
  ...
  const columns: TableColumnList = [
    ...
    },
    {
      ...
      prop: "deptId",
      minWidth: 90,
      formatter: ({ deptId }) => findPathById(treeData.value, deptId)
    },
    {
      label: "状态",
      prop: "status",
  ...
  async function onSearch() {
    loading.value = true;
    const { code, msg, data } = await getAdminPage(toRaw(form));
    if (code !== 200) {
      loading.value = false;
  ...
  onMounted(async () => {
    ...
    // 归属部门
    const { data } = await getAdminDeptTree();
    deptData.value = data;
    higherDeptOptions.value = handleTree(data);
    treeData.value = higherDeptOptions.value;
    treeLoading.value = false;

    // 角色列表
    roleOptions.value = (await getAdminRoleList()).data;
  });
